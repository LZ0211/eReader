...
function Thread(options){
    function route(i){
        let next = ()=>{
            quest.finished.push(quest.running[i])
            setImmediate(()=>route(i))
        }
        if(interval>0){
            next = ()=>{
                quest.finished.push(quest.running[i])
                setTimeout(()=>route(i),interval)
            }
        }
        if(queue.length === 0){
            if(running > 0){
                running -= 1;
                quest.running[i] = null;
    ...
    return {
        ...
        start(){
            ...
            for(let i=0;i<nums;i++){
                log(`[${label}thread ${i}] started`);
                if(interval > 0){
                    setTimeout(route.bind(null,i),interval*i);
                }else{
                    setImmediate(route.bind(null,i));
                }
                running += 1;
            }
            return this;
